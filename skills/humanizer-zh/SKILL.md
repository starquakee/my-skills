...
6. **不要使用分号** - 最终改写文本中不要出现中文分号 `；` 或英文分号 `;`。用句号、逗号、顿号、冒号或换行拆开。
...
**问题：** 分号常让句子显得书面、僵硬，容易暴露机器式整理痕迹。中文改写时尤其不要用 `；`，英文或代码以外的文本也不要用 `;`。

**改写前：**
> 这个功能很实用；它可以减少重复操作；同时让团队更容易协作。

**改写后：**
> 这个功能很实用。它能减少重复操作，也让团队更容易协作。
